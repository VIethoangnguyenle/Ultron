...
- `understand-anything` MCP is enabled (22 tools) and holds exactly **2 graphs**: `vietbank-sme` (8,339 nodes / 27,666 edges / 36 domains, coverage **69%**, 5,179 files, built 2026-09-09, commit `67dbbbd`) and `Vietbank Digital` (15,210 nodes, coverage 97%). **There is NO `dvnh-common` graph on disk** (older notes claimed one) — never cite it.
- **Trace protocol (graph first, source second)**: `list_projects` → `search_by_file_path` / `query_nodes` to locate → `trace_call_chain`, `find_entry_points`, `find_impact`, `get_node_source` to follow → `get_domain_flow_detail` / `get_domain_detail` for business flows. Only then confirm with grep/`read_file` on the exact file. Because coverage is 69%, **an empty graph answer is not proof the code is absent** — fall back to source and say which path you used.
- Why this is a rule, not a preference: on 2026-09-14 the logs showed the graph was effectively never queried (5 × `list_projects`, 1 × `query_nodes`) while one thread burned 452 tool results for 39 questions via grep chains; each tool result re-sends the whole context to a provider that has **no prompt cache**, so grep-first is both less accurate and far more expensive.
- **Branch rule for any graph rebuild (Hoàng, 2026-09-14):** the `vietbank-sme` knowledge graph is ALWAYS built from the branch **`dev-sit` of `vietbank-sme-omni`** and the branch **`dev` of the eKYC repo (`viet-bank-ekyc-sme`)** — never from whatever feature branch happens to be checked out (`feature/goi-3.1-napas2.0`, `pilot_hotfix_13_08`, ...). `dvnh-common` → **the tag matching `common_version` declared in `vietbank-sme-omni`'s `dev-sit` `gradle.properties`** (read it fresh each rebuild: `git grep -n common_version origin/dev-sit -- gradle.properties`; on 2026-09-14 it was `5.0.9` → tag `v5.0.9`), never the branch currently checked out (`feature/kafka-module`). Branch/tag names go stale: re-confirm them before each rebuild.
- Freshness caveat: `get_graph_metadata` returns `status: UNKNOWN` because `meta.json` stores `lastAnalyzedAt` while the server reads `analyzedAt`; after any rebuild make sure that key exists, otherwise staleness cannot be detected.
