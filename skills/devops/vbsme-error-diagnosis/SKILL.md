...
- Constant names are usually self-explanatory: `BANK_NOT_FOUND` = không tìm thấy ngân hàng, `INVALID_AMOUNT` = số tiền không hợp lệ. Translate them even before reading source.
- Two error layers: internal 6-digit codes (vbsme business) vs `VPG*` codes (VNPay payment gateway, from `VnpayPaymentCheckerUtil.formatErrorCode` = `VPG` + action prefix + response code). Don't confuse them.
- If the tester pastes a full JSON error response, first extract the `code` field, then diagnose that.
