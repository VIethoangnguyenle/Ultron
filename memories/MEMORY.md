...
§
Đổi máy: cấp lại credentials — .env + auth.json không sync git; chạy `hermes setup` hoặc copy backup.
§
...
§
Google Chat chỉ đẩy event khi bot được @mention (tin ko @ ko bao giờ tới); adapter tự hồi cứu 8 tin trước trong thread + chèn [NGỮ CẢNH] (context_backfill).
§
...
§
Chẩn đoán hành vi trong Chat: đọc lại lịch sử bằng scripts/gchat_dump.py (read token, chỉ đọc).